...
class MemeEditorViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        // such as in the simulator
        cameraButton.enabled = UIImagePickerController.isSourceTypeAvailable(UIImagePickerControllerSourceType.Camera)
        // Subscribe to the keyboard notifications to raise the view when necessary
        self.subscribeTokeyboardNotifications()
        
        // Hide tabBar.  Do not need it on this screen.
    ...
    override func viewDidLoad() {
        ...
        //       defaultTextAttributes need to be set before textAlignment.
        //       Otherwise, the text is not centered.
    ...
    // Clear the text field when user taps on it.
    // If the text is entered by the user, do not clear it.
    func textFieldDidBeginEditing(textField: UITextField) {
        // Top text field is tagged as 0 in storyboard
        // Bottom text field is tagged as 1 in storyboard
        if textField.tag == 0 {
    ...
    func saveMeme() {
        var meme = Meme(topText: topTextField.text, bottomText: bottomTextField.text, origImage: imageView.image!, memedImage: self.memedImage)
        (UIApplication.sharedApplication().delegate as! AppDelegate).memes.append(meme)
    }
    
    // This is the completionWithItemsHandler, which is called when the ActivityViewController has completed its task.
    // It saves the Meme and dismiss the view controller.
